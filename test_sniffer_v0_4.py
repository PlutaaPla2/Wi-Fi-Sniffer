...
MAC analysis & IE fingerprinting (always on, no flag needed):
  ...
  - Looks up manufacturer name for real (non-randomized) MACs via Scapy's
    built-in IEEE OUI database
...
def lookup_vendor(mac):
    ...
    Scapy ships with a built-in IEEE OUI database (conf.manufdb), so we
    don't need to maintain our own lookup table.
    """
    try:
...
def main():
    ...
    try:
        ...
        # Record start time immediately before handing control to sniff().
        # We declare it global so print_summary() and export_csv() can read it.
        ...
        # Record stop time as soon as sniff() exits (for any reason).
