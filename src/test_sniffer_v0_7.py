...
CSV export (automatic, no flag needed):
  CSV is always saved after capture to OUTPUT_DIR (set at top of file).
  Filename is auto-generated from scan start time: YYYY_MM_DD_H_M.csv
  e.g. 2026_06_17_14_32.csv
...
# ─── Output directory ─────────────────────────────────────────────────────────
# CSV files are saved here automatically with a timestamp filename.
# Edit this path to wherever you want your results to land.
...
def get_ie_fingerprint(pkt):
    ...
    Implementation note — why this walks raw bytes instead of Scapy layers
    -------------------------------------------------------------------
    Scapy chains IEs together as nested Dot11Elt objects, where each
    element's .payload is meant to be the next Dot11Elt. In theory you can
    walk that chain with elt = elt.payload.getlayer(Dot11Elt).

    In practice, this silently breaks: if Scapy's dissector doesn't fully
    recognise a particular IE (a newer IE Extension type, a vendor-specific
    quirk, or a slightly malformed/truncated capture from the air), it
    stops producing Dot11Elt objects and dumps the remaining bytes into a
    generic Raw layer instead. The old version of this function checked
    `elt.__class__.__name__ == "Dot11Elt"` as its loop condition, so the
    moment it hit that Raw layer it just stopped — even though the actual
    IE bytes were still sitting right there, unparsed. This is why some
    devices were showing fingerprints for only 1 IE (e.g. just "SSID")
    when the real frame had many more.

    The fix: every IE follows a fixed TLV format —
        [1 byte ID][1 byte Length][Length bytes of Data]
    — regardless of whether Scapy's dissector recognises that specific ID.
    So instead of trusting Scapy's object chain, we grab the complete raw
    bytes of the frame body with bytes(pkt) and walk the TLV structure
    ourselves. This recovers every IE that's actually present in the air,
    independent of which ones Scapy happens to have a dissector for.
    """
    ie_ids = []
    vendor_ies = []

    # Get the full raw bytes starting from the first Dot11Elt — this
    # includes every IE's bytes whether Scapy dissected them or left them
    # as Raw/Padding/whatever else.
    ...
    # Manual TLV walk: read ID byte, Length byte, skip Length bytes of data,
    # repeat. This is exactly the structure shown in the IE diagram earlier —
    # [ID][Length][Data][ID][Length][Data]...
    i = 0
    while i + 1 < len(raw_bytes):          # need at least an ID and Length byte left
        ie_id  = raw_bytes[i]
        ...
        if data_end > len(raw_bytes):
            # Length byte claims more data than actually remains —
            # frame is truncated here. Stop safely rather than reading
        ...
        i = data_end  # advance past this IE's data to the next ID byte
...
def main():
    ...
    try:
        ...
        # ── Auto-generate CSV filepath from scan start time ──────────────────
        ...
        #   %H = hour (24h)    e.g. 14
        #   %M = minute        e.g. 32
        # scan_start is set just before sniff() begins, so this timestamp
        # reflects when the capture session started, not when it ended.
        csv_filename = scan_start.strftime("%Y_%m_%d_%H_%M_%S") + ".csv"

        # os.makedirs() creates the full directory path if it doesn't exist.
        # exist_ok=True means it won't raise an error if it's already there —
        # safe to call unconditionally every time.
        os.makedirs(OUTPUT_DIR, exist_ok=True)

        # os.path.join() builds a proper file path for the OS you're running on.
        # On Linux this gives e.g. "/home/pi/wifi_scans/2026_06_17_14_32.csv"
        csv_filepath = os.path.join(OUTPUT_DIR, csv_filename)

        export_csv(csv_filepath)
