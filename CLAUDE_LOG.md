...
- The `if raw_fingerprint else ""` arm at `:616` was kept structurally intact — the highest-risk line in the change. A prefix-then-append rewrite would make IE-less frames emit a truthy `"fp2:"`, a fingerprint shared by every such frame, scoring **+3** at `:850` on merges backed by no evidence. Verified: both the no-IE frame and the volatile-tags-only frame yield `''`.
